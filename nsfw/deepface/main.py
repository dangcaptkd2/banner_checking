...
from nsfw.deepface.deepface.commons.functions import normalize_input
from nsfw.deepface.deepface.DeepFace import build_model
from nsfw.deepface.deepface.commons.functions import preprocess_face, find_input_shape
from nsfw.deepface.deepface.DeepFace import build_model

model = build_model('Facenet512')
input_shape_x, input_shape_y = find_input_shape(model)

path_db = './nsfw/deepface/database/'
# a = np.load('./database/nguyen_xuan_phuc.npy')
# b = np.load('./database/pham_minh_chinh.npy')
db = {}
for filename in os.listdir(path_db):
    name = filename[:-4]
    db[name] = np.load(f'./nsfw/deepface/database/{filename}')
